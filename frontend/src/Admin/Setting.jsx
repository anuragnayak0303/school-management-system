      ...
      <main className="w-full md:ml-64 min-h-screen bg-gray-100">
        ...
        <div className="p-4 sm:p-6">
          <div className="flex flex-col lg:flex-row gap-4">
            {/* Settings Navigation */}
            <div className={`w-full  lg:w-1/4 border ${location.pathname.startsWith('/school/teacher/setting')? "h-[20vh]":'h-[29vh]'} border-gray-200 rounded-lg p-4 bg-white shadow-sm`}>
              {links.map(({ name, path }) => (
                <NavLink
                  key={path}
                  to={path}
                  className={({ isActive }) =>
                    `block px-4 py-3 rounded-md text-sm font-medium ${location.pathname.startsWith('/school/teacher/setting') && path =="LogoEdit" && "hidden"} ${isActive
