    ...
    <div className="bg-gray-50 shadow-md border border-gray-300 rounded h-full w-full">
      ...
      <div className="space-y-3 p-4 h-[43vh] overflow-y-auto">
        {leaves.map((req, idx) => (
          <div
            key={idx}
            className={`flex items-start bg-gray-50 border border-gray-300 shadow-sm rounded p-3 gap-3 ${req?.status !== "Pending" && "hidden"}`}
          >
            <img
            ...
            <div className="flex-1 space-y-1">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 text-white">
                  <button onClick={() => handleStatusUpdate(req._id, "Approved")} className="w-5 h-5 bg-green-500 rounded flex items-center justify-center text-[10px]">
                    <FaCheck />
                  </button>
                  <button onClick={() => handleStatusUpdate(req._id, "Rejected")} className="w-5 h-5 bg-red-500 rounded flex items-center justify-center text-[10px]">
                    <FaTimes />
                  </button>
                </div>
              </div>
