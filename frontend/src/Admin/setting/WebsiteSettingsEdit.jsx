...
export default function WebsiteSettingsEdit({ data, onCancel, onSave }) {
  ...
  const [schoolInfo, setSchoolInfo] = useState(data.schoolInfo || {
    name: "", principal: "", phone: "", email: "", address: "", description: ""
  });
  const [marqueeText, setMarqueeText] = useState(data.marqueeText || "");
  ...
  const handleSubmit = async (e) => {
    ...
    // Only append if changed (dataUrls start with data:image)
    ["dark", "light", "favicon"].forEach(key => {
      const value = logos[key];
      if (value?.startsWith("data:image")) {
        formData.append(key, dataURLtoFile(value, `${key}.png`));
      }
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto p-6 md:p-10 bg-white space-y-10 animate-fade-in">
      <h1 className="text-4xl font-extrabold drop-shadow">Edit Website Branding</h1>
      ...
      <div className="max-w-md mx-auto flex justify-between gap-4">
        <button
          type="button"
          onClick={onCancel}
          className="w-1/2 py-3 bg-gray-300 rounded-lg font-semibold hover:bg-gray-400 transition"
          disabled={loading}
        >
          Cancel
        </button>
        <button
          type="submit"
          disabled={loading}
          className={`w-1/2 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition ${loading ? "opacity-70" : ""}`}
        >
